$(document).ready(function () {
          ...
          <a class="list-group-item list-group-item-action" aria-current="true">
                <div class="miImagen misMensajes d-flex w-100 justify-content-between">
                    <div class="container">
                        ...
                        <div class="d-flex flex-row">
                            ...
                            <div class="flex-fill">
                                <p class="fs-5 pt-2 fw-bold">` +
              items[i].nombre +
              `</p>
                            </div>
                            <div class="d-flex justify-content-end">
                                <p class="mb-1 mt-1"><button type="button" class="btn editarCategoria btn-primary" data-bs-toggle="modal" data-bs-target="#miModalEditarCategoria" id="` +
              items[i].Categoria_id +
              `"><i class="bi bi-pen"></i></button></p>
                                <p class="mb-1 mt-1"><button type="button" class="btn borrarCategoria btn-danger" id="` +
              items[i].Categoria_id +
              `"><i class="bi bi-trash"></i></button></p>
                ...
                <div class="d-flex flex-row justify-content-between p-2">
                    <small class="text-muted p-1">Núm. Cursos: ` +
              items[i].noCursos +
              `</small>
         ...
         <a class="list-group-item list-group-item-action" aria-current="true">
              ...
              `"  class="pfp rounded-circle">
                            <div class="align-self-center">
                                <p class="fs-5 ps-3 pt-3 fw-bold align-middle">` +
              items[i].correo +
              `</p>
                            </div>
                        </div>
                        <div class="align-self-start">
                            <small class="text-muted p-3">Fecha Bloqueo: ` +
              items[i].ultimoCambio +
              `</small>
                        </div>
                    </div>
                    <hr class="solid">
                    <div class="d-flex justify-content-end">
                        <p class="mb-1"><button type="button" class="btn btn-success desbloquearUsuario" id="` +
              items[i].Usuario_id +
              `">Desbloquear</button></p>
                    </div>
        ...
        for (let i = 0; i < items.length; i++) {
          $('#misMetodosPagoDashboard').append(
            `            <a class="p-4 list-group-item list-group-item-action" aria-current="true">
              ...
              `" class="btn eliminarMetodo btn-danger"><i class="bi bi-trash"></i></button>
                    </p>
  ...
  function funcDesbloquearUsuario() {
    ...
    if (confirm('¿Seguro que deseas desbloquear este usuario?')) {
      $.ajax({
        type: 'POST',
        data: { funcion: 'actualizarBloqueo', Usuario_id: miIdUsuario },
        url: 'php/API/Usuario.php',
      })
        .done(function () {
          cargarUsuariosAdmin()
    ...
    if (confirm('¿Seguro deseas eliminar este método de pago?')) {
      $.ajax({
        ...
        .done(function (data) {
          ...
          if (data && data.error) {
            alert(
              'Se produjo un error al eliminar el método de pago: ' + data.error
            )
          } else {
            cargarMetodosPago()
